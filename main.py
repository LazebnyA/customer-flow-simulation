...
def get_simulation_parameters():
    ...
    service_rate_value = 3600 // int(input("Скільки часу в середньому займає вибір клієнта (в секундах): "))
    serving_time = int(input("Скільки часу займає приготування кави (в секундах): "))
...
def run_simulation_menu(env, simulation_time, start_time, n_machines, arrival_rate_list, service_rate_value,
    ...
    if user_input == "1":
        ...
        print(f"Середній час очікування клієнта: {Helper.normalize_time_value(metrics.get_mean_waiting_time())} \n")
...
def optimize_parameter_menu(*params_list):
    ...
    print()
    print("Введіть режим оптимізації (з введенням вагових параметрів альфа і бета чи за замовчуванням alpha=beta=1) ")
    print(
        "1. За замовчуванням. Мінімізація середнього часу очікування клієнта + середнього часу очікування автомата на "
        "клієнта.")
    print("2. З можливістю обрати вагові параметри.")
    mode_input = int(input("\n"))

    if mode_input == 1:
        Optimization.optimize(1, 1, range(1, n_max + 1), *params_list)
    elif mode_input == 2:
        alpha = float(input("Введіть параметр альфа, що визначає пріоритет мінімізації середнього часу очікування "
                            "клієнта"
                            "на своє замовлення: alpha = "))
        beta = float(input("Введіть параметр бета, що визначає пріоритет мінімізації середнього часу очікування "
                           "каво-автомата"
